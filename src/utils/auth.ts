...
export const checkPassword = (input: string): boolean => {
  ...
  if (!correctPassword) {
    console.error('VITE_WEBSITE_PASSWORD environment variable not set. Password protection will not work.');
    return false; // Always fail if no password is configured
  }
